...
def draw_gt_boxes3d(
    ...
    Returns:
        fig: updated fig
    """
    num = len(gt_boxes3d)  #得到有多少个boxes需要画出来
    for n in range(num):
        b = gt_boxes3d[n]  #得到其中一个box
        if color_list is not None:
            color = color_list[n]  #设置画出的box的颜色
        if draw_text:
            mlab.text3d(   #设置的text显示的位置在第四个坐标的位置
                b[4, 0],  
                b[4, 1],
                b[4, 2],
                label,
        ...
        for k in range(0, 4):  #0 1 2 3  
            ...
            i, j = k, (k + 1) % 4  #画底面
            mlab.plot3d(
                [b[i, 0], b[j, 0]],   #(b[0,0]  b[1,0])
                [b[i, 1], b[j, 1]],   #(b[0,1]  b[1,1])
                [b[i, 2], b[j, 2]],   #(b[0,2]  b[1,2])
                color=color,
                tube_radius=None,
            ...
            i, j = k + 4, (k + 1) % 4 + 4  #(4,5,6,7)  (5,6,7,4) #画上面
            mlab.plot3d(
                [b[i, 0], b[j, 0]],#(b[])
                [b[i, 1], b[j, 1]],
                [b[i, 2], b[j, 2]],
            ...
            i, j = k, k + 4  #(0,1,2,3)  (4,5,6,7)    #画周围的4条棱
            mlab.plot3d(
                [b[i, 0], b[j, 0]],
                [b[i, 1], b[j, 1]],
                [b[i, 2], b[j, 2]],
...
def draw_xyzwhl(
    ...
    for n in range(num):
        print(gt_boxes3d[n])
        box6 = gt_boxes3d[n]   #得到一个gt_boxes3d
        b = xyzwhl2eight(box6)  #得到图片中的  boxes
        if rot:
            b = b.dot(rotz(box6[7]))
            # b = b.dot(rotx(box6[6]))
            # print(rotz(box6[6]))
